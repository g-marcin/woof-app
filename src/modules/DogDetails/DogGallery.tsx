                ...
                if (entries[0].isIntersecting && !isLoadingMore) {
                    ...
                    setTimeout(() => {
                        setDisplayedCount(prev =>
                            Math.min(prev + IMAGES_PER_PAGE, imageList.length)
                        )
                        setIsLoadingMore(false)
                    }, 2000)
                }
                ...
                <div className="flex flex-wrap gap-4 py-5 max-md:gap-2.5">
                    ...
                    {isLoadingMore &&
                        Array.from({
                            length: Math.min(
                                IMAGES_PER_PAGE,
                                imageList.length - displayedCount
                            ),
                        }).map((_, index) => (
                            <div
                                key={`skeleton-${index}`}
                                className="relative flex-[1_1_calc(25%-12px)] min-w-[200px] pt-[calc(25%-12px)] overflow-hidden rounded-lg bg-[color:var(--secondary)] shadow-[0_2px_8px_rgba(0,0,0,0.1)] max-md:flex-[1_1_calc(50%-8px)] max-md:pt-[calc(50%-8px)] max-sm:flex-[1_1_100%] max-sm:pt-[100%]"
                            >
